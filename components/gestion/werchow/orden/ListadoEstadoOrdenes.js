...
const ListadoEstadoOrdenes = ({
  ...
  if (listado.length === 0)
    return (
      <div className="container border border-dark alert alert-info text-center text-uppercase mt-4 mb-4">
  ...
  return (
    <div className="container list mt-4 border border-dark p-4">
      <h2>
        <strong>
          <u>Estado Ordenes de Pago</u>
        </strong>
      </h2>

      <div className="alert alert-info border border-dark mt-4 mb-4 text-center text-uppercase">
        Las ordenes de pago que no esten pagadas, saldran en color rosado. Por
        lo contrario, se pondran en color verde. Para marcalas como pagadas,
        primero deben estar autorizadas. De esta manera aparecera un boton verde
        con un visto, al cual haciendole click se tildara como pagada. Las
        ordenes que aparezcan en rojo con letras, estan anuladas.
      </div>

      <div id="list" className="border border-dark mt-4 p-1 mt-4">
        <ReactTable
          data={listado}
          filterable
          ...
          columns={[
            {
              ...
              columns: [
...

                {
                  ...
                  Cell: (row) => (
                    <div>
                      {row.original.pagado === 1 ? null : (
                        <button
                          className="btn btn-success btn-sm mr-1"
                          onClick={() => {
                            updatePagadas(
                              row.original.idorden,
                              row.original.norden
                            );
                          }}
                        >
                      ...
                      {row.original.tipo_orden === "Ordenes Medica" ||
                      row.original.tipo_orden === "Practicas Medica" ? (
                        <button
