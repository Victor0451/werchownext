...
const stock = () => {
  ...
  const historialParcelas = async (id) => {
    guardarhistorial([]);

    await axios
      .get(`${ip}api/sepelio/parcelas/historialparcelas`, {
        params: {
          id: id,
        },
      })
      .then((res) => {
        if (res.data.length > 0) {
          ...
          toastr.info("Esta parcela no tiene lugares asignados");
        }
      })
      .catch((error) => {
        console.log(error);
        toastr.error("Ocurrio un error al traer el historial de la parcelas");
      });
  };
